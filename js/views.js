  ...
  FC.views.onboarding = function () {
    ...
    const brand = `<div class="wl-brand">
      ...
      <div class="wl-foot">Proyecto de fans no oficial · sin afiliación con EA Sports / FIFA · sin dinero real
        <div style="margin-top:6px"><button type="button" class="wl-link" id="wl-privacy-foot" style="display:inline">Privacidad y aviso legal</button></div>
      </div>
    </div>`;
      ...
      panel = `<div class="wl-panel">
        ...
        ${!obCodeSent
          ? `<p class="wl-sub">Te enviaremos un código de acceso de un solo uso. Si ya tienes cuenta, recuperarás tus carreras.</p>
             ...
             <label style="display:flex;align-items:flex-start;gap:8px;margin:2px 0 14px;font-size:12.5px;color:var(--text-dim);cursor:pointer"><input type="checkbox" id="wl-consent" style="margin-top:2px;flex:none" /> <span>He leído y acepto la <button type="button" class="wl-link" id="wl-privacy-link" style="display:inline">Política de privacidad</button> y el uso de mi email para el acceso.</span></label>
    ...
    if (obStep === "home") {
      ...
      if ($("wl-send")) $("wl-send").addEventListener("click", async () => {
        ...
        if (!$("wl-consent") || !$("wl-consent").checked) { UI.toast("Debes aceptar la política de privacidad", "err"); return; }
  ...
  FC.views.cloud = function () {
    ...
    if (!configured) {
      ...
      if ($("cl-send")) $("cl-send").addEventListener("click", busy("cl-send", async () => {
        ...
        if (!$("cl-consent") || !$("cl-consent").checked) { UI.toast("Debes aceptar la política de privacidad", "err"); return; }
